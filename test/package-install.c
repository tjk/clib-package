...
main() {
  describe("clib_package_install") {
    ...
    it("should install from git URI") {
      clib_package_t *pkg = clib_package_new_from_slug("https://github.com/clibs/sds.git@2.0.0", 0);
      assert(pkg);
      assert(0 == clib_package_install(pkg, "./test/fixtures/", 0));
      assert(0 == fs_exists("./test/fixtures/"));

      assert(0 == fs_exists("./test/fixtures/sds/"));
      assert(0 == fs_exists("./test/fixtures/sds/package.json"));
